...
import {convertToPdf} from '../../engines/pdf-processing_engine.js';
...
export default function ImagePdfConverter({
  ...
  (async () => {
    try {
      ...
      const success = await convertToPdf(url.trim().replace(/\\/g, "/"), finalPath.replace(/\\/g, "/"));
      ...
      if (success) {
        ...
        setFatalError('PDF conversion failed. Please try again.');
      }
    } catch (e: any) {
      if (!isSubscribed) return;
      
      setFatalError(e?.message || 'PDF conversion failed. Please try again.');
      setIsRunning(false);
      setTimeout(() => {
        exit();
      }, 3000);
                ...
                {processConfirmation.completed ? (
                    <Box flexDirection="column" gap={1}>
                        <Alert variant="success">
                            PDF conversion successfully ! Find it here : {downloadPath}
                        </Alert>
