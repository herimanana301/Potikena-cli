...
import downloader from '../../engines/mp3-downloader.js';
...
export default function Mp3Downloader({
  ...
  (async () => {
    try {
      ...
      const success = await downloader(url.trim(), finalPath);
  ...
  return () => {
	...
	return (
            <Box flexDirection="column" gap={1}>
                <TextInput
                    placeholder="Enter the video URL here (type 'exit' to go back)"
                    onChange={value => {
                        setUrl(value);
                ...
                {processConfirmation.completed ? (
                    <Box flexDirection="column" gap={1}>
                        <Alert variant="success">
                            Converted MP3 downloaded successfully to : {downloadPath}
                        </Alert>
